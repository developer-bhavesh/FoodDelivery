...
class MainViewModel: ObservableObject {
    ...
    @Published var selectedZone = "Rajasthan"
    @Published var selectedArea = "Pali"
    ...
    func addToCart(product: Product) {
            if let index = cart.firstIndex(where: { $0.id == product.id }) {
                cart[index].quantity += 1  // Increase quantity if product exists
            } else {
                var newProduct = product
                newProduct.quantity = 1
                cart.append(newProduct)  // Add new product
            }
        }
        ...
        func removeFromCart(product: Product) {
            if let index = cart.firstIndex(where: { $0.id == product.id }) {
                if cart[index].quantity > 1 {
                    cart[index].quantity -= 1  // Decrease quantity if > 1
                } else {
                    cart.remove(at: index)  // Remove product if quantity is 1
                }
            }
        }
